...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/02/02 15:05:06 by fursini           #+#    #+#             */
/*   Updated: 2023/02/07 21:35:50 by fursini          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_brokenl(char *brokenl)
{
	...
	newbl = (char *)malloc(sizeof(char) * (ft_strlengnu(brokenl) - i + 1));
	if (!newbl)
		return (NULL);
	while (brokenl[i])
		newbl[j++] = brokenl[i++];
...
char	*get_broken_line(int fd, char *brokenl)
{
	...
	while (rlen != 0 && ft_strchr(brokenl, '\n') == 0)
	{
		...
		buf[rlen] = 0;
		brokenl = ft_strjoingnu(brokenl, buf);
	}
	free (buf);
	return (brokenl);
}
